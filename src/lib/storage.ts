// src/lib/storage.ts (remote-only, versioned saves, tournament + list helpers)
 ...
 * List Game types
 * ========================= */
export type Table = { a?: string; b?: string }; // player ids
...
export async function findByCodeRemote(code: string): Promise<string | null> {
  try {
    const { data } = await api<{ id: string }>(`/api/by-code/${encodeURIComponent(code)}`);
    return data.id || null;
  } catch {
    return null;
  }
...
/* =======================================================
 * LIST GAME: remote CRUD (using /api/create and /api/list)
 * ======================================================= */

export async function createListRemote(payload: {
  name: string;
  hostId: string;
}): Promise<{ id: string; code: string }> {
  // /api/create must accept { type: 'list' }
  const { data } = await api<{ ok: true; id: string; code: string; type: "list" }>(
    "/api/create",
    { method: "POST", body: JSON.stringify({ ...payload, type: "list" }) }
  );
...
/* =======================================================
 * LIST GAME: queue/table helpers (mutate + save, like tournaments)
 * ======================================================= */

// fills any empty seat in each table with next from queue
...
export async function listSetTables(x: ListGame, count: 1 | 2): Promise<ListGame> {
  ...
  if (count === 1) {
    // If shrinking to 1, push any displaced players back into queue (preserve order)
    ...
    // expanding to 2 (keep existing as first)
...
export async function listILost(x: ListGame, tableIndex: number, loserId: string): Promise<ListGame> {
  ...
  // winner remains seated; refill only the empty seat(s)
  fillTablesSync(copy);
  const saved = await saveListRemote(copy);
  Object.assign(x, saved);
  return saved;
