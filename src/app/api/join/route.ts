...
type Tournament = {
  id: string; name: string; code?: string; hostId: string; status: "setup" | "active" | "completed";
...
export async function POST(req: Request) {
  ...
  if (!mappingRaw) mappingRaw = await env.KAVA_TOURNAMENTS.get(`code2:${code}`);
  if (!mappingRaw) return NextResponse.json({ error: "Invalid code" }, { status: 404 });

  let type: "tournament" | "list" = "tournament";
  let id = mappingRaw;
  try {
    const parsed = JSON.parse(mappingRaw) as { type?: "tournament" | "list"; id?: string };
    if (parsed?.id) { id = parsed.id; type = parsed.type === "list" ? "list" : "tournament"; }
  } catch {}
  ...
  if (type === "tournament") {
    ...
    const already = t.players.some(p => p.id === player.id) || t.pending.some(p => p.id === player.id);
    if (!already) {
  ...
  if (!g.players.find(p => p.id === player.id)) g.players.push({ id: player.id, name: player.name });
  const sitting = g.tables.some(tb => tb.a === player.id || tb.b === player.id) || g.queue.includes(player.id);
