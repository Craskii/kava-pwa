...
export default function Client() {
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = () => {
      ...
      catch { const img: any = new Image(); img.src = url; }
  ...
  const renamePlayer = (pid: string) => {
    ...
    scheduleCommit(d => {
      const p = d.players.find(pp => pp.id === pid); // <-- fixed selector (pp.id)
      if (p) p.name = v;
    });
  };
...
const input: React.CSSProperties = { width:260, maxWidth:"90vw", padding:"10px 12px", borderRadius:10, border:"1px solid #333", background:"#111", color:"#fff" } as any; // <-- fixed quotes here
