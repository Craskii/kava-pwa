...
function coerceList(raw: any): ListGame | null {
  ...
  try {
    ...
    // Detect if the doc already uses the new single-queue schema
    ...
    if (hasSingleQueue) {
      ...
      // Legacy: merge old queues (order preserved: q8 then q9, unique)
    ...
    if (raw.prefs && typeof raw.prefs === 'object') {
      ...
      // ⚠️ ONLY infer from legacy queues if the doc truly has no single queue.
      // This prevents toggles from being overwritten back to 8-ft/9-ft.
      const q8 = Array.isArray(raw.queue8) ? raw.queue8.map((id: any) => String(id)) : [];
      const q9 = Array.isArray(raw.queue9) ? raw.queue9.map((id: any) => String(id)) : [];
      q8.forEach((id) => {
        const pid = String(id);
        if (has(pid)) prefs[pid] = '8 foot';
      });
      q9.forEach((id) => {
        const pid = String(id);
        if (has(pid)) prefs[pid] = '9 foot';
      });
    }
    // Clean seats that reference missing players
    tables.forEach((t) => {
...
async function putList(doc: ListGame) {
  await fetch(`/api/list/${encodeURIComponent(doc.id)}`, {
    ...
    body: JSON.stringify({ ...doc, schema: 'v2', queue8: [], queue9: [] }), // also clear legacy fields
  });
}
    ...
    <main style={wrap}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginTop: 6 }}>
        <div>
          ...
          <div style={{ opacity: 0.8, fontSize: 14, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {g.tables.map((t, i) => {
            ...
            return (
              <div key={i} style={{ background: '#0b3a66', borderRadius: 12, padding: '12px 14px', border: '1px solid rgba(56,189,248,.35)' }}>
                <div style={{ opacity: 0.9, fontSize: 12, marginBottom: 6 }}>{t.label === '9 foot' ? '9-Foot Table' : '8-Foot Table'} • Table {i + 1}</div>
              ...
              return (
                ...
                >
                  ...
                  {canEditPref ? (
                    ...
                    <small style={{ opacity: 0.7, width: 48, textAlign: 'right' }}>{pref === 'any' ? 'Any' : pref === '9 foot' ? '9-ft' : '8-ft'}</small>
                  )}
                </li>
              );
            })}
