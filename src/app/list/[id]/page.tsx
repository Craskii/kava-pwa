...
export default function ListLobby() {
  ...
  // derive id from URL on the client
  const id =
  ...
  // SSE stream (with guard to ignore malformed snapshots)
  useEffect(() => {
    if (!id) return;
    ...
    try {
      ...
      es.onmessage = (e) => {
        try {
          ...
          // prefer `list` key from server, but accept direct doc too
          ...
          // ignore parse errors; next tick will retry
        }
      };
    } catch (err) {
  ...
  return (
    <main style={wrap}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginTop: 6 }}>
        <div>
          ...
          <div style={{ opacity: 0.8, fontSize: 14, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            Private code: <b>{g.code || '—'}</b> {g.code && <button style={chipBtn} onClick={onCopy}>Copy</button>} • {safePlayers.length}{' '}
      ...
      <section style={notice}>
        <b>How it works:</b> One shared queue feeds both tables. When someone taps <i>“I lost”</i>, the next person in the
        queue sits at whichever table frees up first.
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px,1fr))', gap: 12 }}>
          {safeTables.map((t, i) => {
            ...
            return (
              <div
                key={i}
                style={{ background: '#111', borderRadius: 12, padding: '10px 12px', border: '1px solid rgba(255,255,255,.12)' }}
              >
...
const btnActive: React.CSSProperties = { padding: '10px 14px', borderRadius: 10, border: 'none', background: '#0ea5e9', color: '#fff', fontWeight: 700 };
