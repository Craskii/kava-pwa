...
/* ---------- coerce remote doc to a safe shape ---------- */
function coerceList(x: any): ListGame | null {
  ...
  try {
    return {
      ...
      players: Array.isArray(x.players) ? x.players.map((p: any) => ({
        id: String(p?.id ?? ''),
...
export default function ListLobby() {
  ...
  // derive id from URL on client (keeps this page "client only")
  const id =
  ...
  /* ---------- bump alerts if my seat changes ---------- */
