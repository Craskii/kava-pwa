...
export default function ListLobby() {
  ...
  // alert banners (works for players & host)
  useQueueAlerts({
    listId: id,
  ...
  // Live updates: SSE with fallback to smart poll
  useEffect(() => {
    if (!id) return;
    let es: EventSource | null = null;
    let poll: { stop: () => void; bump: () => void } | null = null;

    const startPoll = () => {
      if (poll) return;
      poll = startSmartPoll(async () => {
        try {
          const next = await getListRemote(String(id));
          const coerced = coerceList(next);
          setG(coerced);
  ...
  // Auto-seed tables for host if this is an old/empty document
  useEffect(() => {
    if (!g) return;
    const iAmHost = g.hostId === me.id;
    const noTables = !Array.isArray(g.tables) || g.tables.length === 0;
    if (iAmHost && noTables && !busy) {
      (async () => {
        setBusy(true);
        try {
          const updated = await listSetTables(g, 1);
  ...
  }, [g?.id, g?.tables?.length, g?.hostId, me.id, busy]);
    ...
    return (
      <main style={wrap}>
        ...
        <div style={errorBox}>
          ...
          <div style={{ opacity: 0.85, fontSize: 12, marginTop: 6 }}>{err}</div>
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button style={btn} onClick={loadOnce}>
              Retry
            </button>
            <a href="/lists" style={btnGhost}>
              Back to My lists
            </a>
          </div>
        </div>
  ...
  return (
    <main style={wrap}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginTop: 6 }}>
        <div>
          ...
          <div style={{ opacity: 0.8, fontSize: 14, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            Private code: <b>{g.code || '—'}</b> {g.code && <button style={chipBtn} onClick={onCopy}>Copy</button>} •{' '}
            {g.players.length} {g.players.length === 1 ? 'player' : 'players'}
          </div>

          {/* helpful debug line; safe to remove */}
          <div style={{ opacity: 0.6, fontSize: 12, marginTop: 6 }}>
            me.id: {me.id} • hostId: {g.hostId} • iAmHost: {String(iAmHost)}
          </div>
        </div>

        <div style={{ display: 'flex', gap: 8 }}>
          {!iAmHost && !seated && !queued && (
      ...
      <section style={notice}>
        <b>How it works:</b> One shared queue feeds both tables. When someone taps <i>“I lost”</i>, the next person in
        the queue sits at whichever table frees up first.
      </section>
      ...
      {iAmHost && (
        <section style={card}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
            <button style={oneActive ? btnActive : btn} onClick={() => onTables(1)} disabled={busy}>
              1 Table
            </button>
            <button style={twoActive ? btnActive : btnGhost} onClick={() => onTables(2)} disabled={busy}>
              2 Tables
            </button>
          </div>
