...
export default function Lobby() {
  ...
  useEffect(() => {
    ...
    const poll = startSmartPoll(async () => {
      ...
      if (res.ok) {
        const vHeader = res.headers.get('x-t-version') || '';
        const next = await res.json();
        const v = Number(vHeader);
        setT({ ...next, v: Number.isFinite(v) ? v : (next.v ?? 0) }); // ✅ carry server version
        return vHeader;
      }
      return null;
    });
          ...
          <div style={card}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:10 }}>
              ...
              {t.status !== 'completed' && (
                <button
                  style={btnGhost}
                  onClick={async ()=>{ if (confirm('Delete tournament?')) { await deleteTournamentRemote(t.id); r.push('/'); r.refresh(); } }}
                >Delete</button>
              )}
