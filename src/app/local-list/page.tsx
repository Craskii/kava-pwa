...
export default function LocalListPage() {
  ...
  function autoSeat(next: ListGame) {
    ...
    for (const t of next.tables) {
      if (t.a) seatedSet.add(t.a);
      if (t.b) seatedSet.add(t.b);
    }
    ...
    next.tables.forEach((t) => {
      if (!t.a)
        t.a =
          takeFromQueue(t.label) ??
          (fillFromPlayersIfNoQueue ? takeFromPlayers(t.label) : undefined);
      if (!t.b)
        t.b =
          takeFromQueue(t.label) ??
          (fillFromPlayersIfNoQueue ? takeFromPlayers(t.label) : undefined);
  ...
  const renameList = (nm: string) => {
    ...
    update(
      ...
      { t: Date.now(), who: me.id, type: 'rename', note: v }
    );
  };
  ...
  const renamePlayer = (pid: string) => {
    ...
    update(
      ...
      { t: Date.now(), who: me.id, type: 'rename-player', note: v }
    );
  };
  const setPrefFor = (pid: string, pref: Pref) =>
    update(
      (d) => {
        d.prefs[pid] = pref;
      },
  ...
  const moveUp = (index: number) =>
    update((d) => {
      ...
      const a = d.queue[index - 1];
      d.queue[index - 1] = d.queue[index];
      d.queue[index] = a;
    });
  ...
  return (
    <main ref={pageRootRef} style={wrap}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginTop: 6 }}>
        <div>
          ...
          >
            ...
            <button style={btnGhostSm} onClick={() => setShowHistory((v) => !v)}>
              {showHistory ? 'Hide' : 'Show'} history
            </button>
          </div>
        </div>
        <div style={{ display: 'grid', gap: 6, justifyItems: 'end' }}>
          {!seated && !g.queue.includes(me.id) && (
            <button style={btn} onClick={joinQueue} disabled={busy}>
              Join queue
            </button>
          )}
          {g.queue.includes(me.id) && (
            <button style={btnGhost} onClick={leaveQueue} disabled={busy}>
              Leave queue
            </button>
          )}
          {players.some((p) => p.id === me.id) && (
            <button style={btnGhost} onClick={leaveList} disabled={busy}>
              Leave list
            </button>
          )}
        </div>
          ...
          {(g.audit?.length ?? 0) === 0 ? (
            ...
            >
              {g.audit!.slice().reverse().map((a, i) => (
                ...
                >
                  <b style={{ opacity: 0.9 }}>{a.type}</b>
                  {a.note ? <span style={{ opacity: 0.85 }}> — {a.note}</span> : null}
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                onClick={() =>
                  update((d) => {
                    if (d.tables.length < 2)
                      d.tables.push({ label: d.tables[0]?.label === '9 foot' ? '8 foot' : '9 foot' });
                  })
                }
              ...
              <button
                ...
                onClick={() =>
                  update((d) => {
                    if (d.tables.length > 1) d.tables = d.tables.slice(0, 1);
                  })
                }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px,1fr))', gap: 12 }}>
          {g.tables.map((t, i) => {
            ...
            return (
              ...
              >
                <div style={{ opacity: 0.9, fontSize: 12, marginBottom: 6 }}>
                  {t.label === '9 foot' ? '9-Foot Table' : '8-Foot Table'} • Table {i + 1}
                </div>
                <div style={{ display: 'grid', gap: 8 }}>
                  <Seat side="a" />
                  <div style={{ opacity: 0.7, textAlign: 'center' }}>vs</div>
                  <Seat side="b" />
                </div>
              </div>
            );
      ...
      <section style={card}>
        ...
        {queue.length === 0 ? (
          <div style={{ opacity: 0.6, fontStyle: 'italic' }}>Drop players here</div>
        ) : (
          <ol
            style={{ margin: 0, paddingLeft: 18, display: 'grid', gap: 6 }}
            onDragOver={supportsDnD ? onDragOver : undefined}
            onDrop={
              supportsDnD ? (e) => handleDrop(e, { type: 'queue', index: queue.length, pid: '__end' as any }) : undefined
            }
          >
            {queue.map((pid, idx) => {
              ...
              return (
                <li
                  ...
                  onDragStart={
                    supportsDnD && iHaveMod ? (e) => onDragStart(e, { type: 'queue', index: idx, pid }) : undefined
                  }
                  onDragOver={supportsDnD ? onDragOver : undefined}
                ...
                >
                  ...
                  {!supportsDnD && iHaveMod && (
                    <div style={{ display: 'flex', gap: 4, marginRight: 6 }}>
                      <button style={btnTiny} onClick={() => moveUp(idx)} disabled={busy || idx === 0} aria-label="Move up">
                        ▲
                      </button>
                      <button
                        style={btnTiny}
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {iHaveMod || canEditSelf ? (
                      <>
                        <button
                          style={pref === 'any' ? btnTinyActive : btnTiny}
                          onClick={(e) => {
                            e.stopPropagation();
                            setPrefFor(pid, 'any');
                          }}
                        ...
                        <button
                          style={pref === '9 foot' ? btnTinyActive : btnTiny}
                          onClick={(e) => {
                            e.stopPropagation();
                            setPrefFor(pid, '9 foot');
                          }}
                          disabled={busy}
                        >
                          9-ft
                        ...
                        <button
                          style={pref === '8 foot' ? btnTinyActive : btnTiny}
                          onClick={(e) => {
                            e.stopPropagation();
                            setPrefFor(pid, '8 foot');
                          }}
                          disabled={busy}
                        >
                          8-ft
                      ...
                      <small style={{ opacity: 0.7, width: 48, textAlign: 'right' }}>
                        {pref === 'any' ? 'Any' : pref === '9 foot' ? '9-ft' : '8-ft'}
                      </small>
                    )}
                  </div>
                </li>
        ...
        <section style={card}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
            <input
              ...
              disabled={busy}
            />
            <button style={btn} onClick={addPlayer} disabled={busy || !nameField.trim()}>
            ...
            {players.map((p) => {
              ...
              return (
                ...
                >
                  <span>
                    {p.name}
                    {isCohost ? <em style={{ opacity: 0.6, marginLeft: 8 }}>(Cohost)</em> : null}
                  </span>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    {!inQueue(p.id)
                      ? iHaveMod && (
                          <button style={btnMini} onClick={() => enqueuePid(p.id)} disabled={busy}>
                            Queue
                          </button>
                        )
                      : iHaveMod && (
                          <button style={btnMini} onClick={() => dequeuePid(p.id)} disabled={busy}>
                            Dequeue
                          </button>
                        )}
                    {(iHaveMod || canEditSelf) && (
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button style={pref === 'any' ? btnTinyActive : btnTiny} onClick={() => setPrefFor(p.id, 'any')} disabled={busy}>
                          Any
                        </button>
                        <button style={pref === '9 foot' ? btnTinyActive : btnTiny} onClick={() => setPrefFor(p.id, '9 foot')} disabled={busy}>
                          9-ft
                        </button>
                        <button style={pref === '8 foot' ? btnTinyActive : btnTiny} onClick={() => setPrefFor(p.id, '8 foot')} disabled={busy}>
                          8-ft
                        </button>
                      </div>
                    )}
                    ...
                    {(iHaveMod || canEditSelf) && (
                      <button style={btnMini} onClick={() => renamePlayer(p.id)} disabled={busy}>
                        Rename
                      </button>
                    )}
                    {iHaveMod && (
                      <button style={btnGhost} onClick={() => removePlayer(p.id)} disabled={busy}>
                        Remove
                      </button>
                    )}
                  </div>
